<template>
    <div class="storage">
       ...
       <h2>cookie</h2> <br>  
        <h3>정의</h3> <br>    
        <li>클라이언트에 저장되는 키와 값이 들어 있는 작은 데이터 파일</li>  
        <li>쿠키에는 이름, 값, 만료날짜(쿠키 저장기간), 경로 정보를 포함한다.</li> 
         <br>
        <h3>목적</h3>  
        <li>클라이언트에 저장되고 서버가 주체가 되어 필요한 정보를 지정한다.</li>  
        <li>또한 HTTP요청마다 메시지에 같이 실어 서버로 전달하는 역할을 한다.</li>  <br>
        <h2>sessionStorage</h2>    <br>
        <h3>정의</h3>   
        <li>현재 세션 동안만 유지되는 스토리지</li>  
        <li>로컬스토리지와는 다르게 데이터가 영구적으로 보관되지 않는다.</li>     
        <h3>목적</h3>  
        <li>HTML5이후 새로 생긴 신기능으로 cookie를 대체하기 위한 목적이 있다.</li>  
        <li>일회성 로그인등에 이용된다.</li> 
        <h2>localStorage</h2> <br>
        <h3>정의</h3>   
        <li>로컬에 Origin 별로 지속되는 스토리지. </li>  
        <li>도메인 마다 별도로 Local Storage 가 생성된다. </li>     
        <h3>목적</h3>  
        <li>HTML5이후 새로 생긴 신기능으로 cookie를 대체하기 위한 목적이 있다.</li> 
         <li> 지속적인 로그인 기능을 이용할 때 이용한다.</li> 
          <br>
           <br>
...
<figure class="highlight js">
    <table>
        <tr><td class="code">
            <pre><span> var setCookie = function(name, value, day) {</span><br><span></span><br>
               <span>
        date.setTime(date.getTime() + day * 60 * 60 * 24 * 1000);</span><br>
               <span>  document.cookie = name + '=' + value + ';expires=' + date.toUTCString() + ';path=/';</span><br>};
               <span>  var getCookie = function(name) {</span><br>
               <span>
       var value = document.cookie.match('(^|;) ?' + name + '=([^;]*)(;|$)');</span><br>
               <span>
       return value? value[2] : null;</span><br>   };
               <span>
    var deleteCookie = function(name) {</span><br>
               <span>
           var date = new Date();</span><br>
               <span>

           document.cookie = name + "= " + "; expires=" + date.toUTCString() + "; path=/";
       }</span><br>
               <span>function Writeall()</span><br>	{
               <span>	   if( document.myform.customer.value == "" ){</span><br>
   <span>     alert("Id를 입력하시오!");</span><br>
   <span>return;</span><br>   }
   <span>
	   setCookie("name", document.myform.customer.value, 3)</span><br>
   <span>
	   alert("쿠키 Cookies : " + "name=" + getCookie("name") );</span><br>
   <span>
     sessionStorage.setItem("name", document.myform.customer.value)</span><br>
   <span>
     alert("localstorage : " + "ID=" + sessionStorage.getItem("name"));</span><br>
   <span>
    localStorage.setItem("name", document.myform.customer.value)</span><br>
   <span>	      
	


    alert("localstorage : " + "ID=" + localStorage.getItem("name"));</span><br>
	}
   <span>  function deleteall(){</span><br>
     <span>localStorage.removeItem("name");</span><br><span>sessionStorage.removeItem("name");</span><br><span> }</span><br>
        ...
        <br>  
        <br>
        <h2>Storage에 저장 확인하기</h2>
        <h4>*위 Storage기능은 Vue로 제작되었음을 알립니다.</h4>
        <br>
        <li>먼저 ID를 입력한 후, 로그인을 누른다. </li>
        <li>F12키를 누르고 Application을 찾아가 Storage를 열어 자신이 입력한 ID가 저장됨을 확인한다.</li>
